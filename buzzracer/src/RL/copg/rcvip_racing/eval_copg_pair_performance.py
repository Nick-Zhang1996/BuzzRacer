...
def getPerformanceMetric(p1_policy_pth, p2_policy_pth):
    ...
    for i in range(2000):
        ...
        #     print(torch.sum(torch.stack(over_mat)), c1_out,c2_out, a_win,b_win, overtakings_p1,overtakings_p2)
            ...
            '''
            p1_col = np.array(c1_out/init_size)
            p1_over = np.array(overtakings_p1/init_size)
            p2_col = np.array(c2_out/init_size)
            p2_over = np.array(overtakings_p2/init_size)

            return p1_col, p2_col, p1_over, p2_over
