...
# print success rate:
print('mppi: total %d, success %d, success rate %.1f%%' % (mppi_total_count,
      mppi_total_count-mppi_bad_count, (mppi_total_count - mppi_bad_count)/mppi_total_count*100))
print('ccmppi: total %d, success %d, success rate %.1f%%' % (ccmppi_total_count, ccmppi_total_count -
      ccmppi_bad_count, (ccmppi_total_count - ccmppi_bad_count)/ccmppi_total_count*100))
...
b_low_thresh = 0
b_high_thresh = 100
mask1 = np.bitwise_and(ccmppi[:, 8] > a_low_thresh,
                       ccmppi[:, 8] < a_high_thresh)
mask2 = np.bitwise_and(ccmppi[:, 9] > b_low_thresh,
                       ccmppi[:, 9] < b_high_thresh)
...
print('mppi  : laptime %.3f, collision %.2f ' %
      (mppi_mean_laptime, mppi_mean_collision))
print('ccmppi: laptime %.3f, collision %.2f ' %
      (ccmppi_mean_laptime, ccmppi_mean_collision))
...
for index in [4]:
    ...
    print('log index: ccmppi: %d,  mppi: %d' %
          (ccmppi[index_cc, 7], mppi[index_mppi, 7]))

    # plot frontier with circled settings
    plt.plot(ccmppi[:, 3], ccmppi[:, 2], 'o', color='cyan', label='CCMPPI')
    plt.plot(mppi[:, 3], mppi[:, 2], 'o', color='orange', label='MPPI')

    plt.scatter(ccmppi[index_cc, 3], ccmppi[index_cc, 2], s=80,
                facecolor='none', edgecolor='r', label='same setting', zorder=10)
    plt.scatter(mppi[index_mppi, 3], mppi[index_mppi, 2],
                s=80, facecolor='none', edgecolor='r', zorder=10)
    plt.xlabel('Number of collisions')
    plt.ylabel('Laptime (s)')
    plt.legend()
    plt.show()
