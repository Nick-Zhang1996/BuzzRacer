...
def run_ukf():
    ...
    for i in range(x.shape[0]-1):
        ...
        ukf.state, ukf.state_cov = ukf.predict(
            ukf.state, ukf.state_cov, control, t[i+1]-t[i])
        measurement = (x[i+1], y[i+1], heading[i+1])
        ukf.state, ukf.state_cov = ukf.update(
            ukf.state, ukf.state_cov, measurement)
...
def test_predict(ukf, show=False):
    ...
    for i in range(100, data_len-lookahead_steps-1-200):
        ...
        vv_real = (vx[i:i+lookahead_steps]**2 +
