...
class Model:
    ...
    def get_curvature(self, s):
        ...
        while (s > map_params[num_segments-1][3] + map_params[num_segments-1][4]).any():
            s[s > map_params[num_segments-1][3] + map_params[num_segments-1][4]
              ] -= map_params[num_segments-1][3] + map_params[num_segments-1][4]
        for ii in range(num_segments):
            truths = np.where(np.logical_and(
                map_params[ii][3] <= s, s <= map_params[ii][3] + map_params[ii][4]))
    ...
    def update_dynamics(self, state, input, dt, nn=None, throttle_nn=None, cartesian=np.array([])):
        ...
        while t < dt:
            ...
            ax = ((fFx * cos(delta) - fFy * sin(delta) + fRx) /
                  m_Vehicle_m + vy * wz)
            ...
            vy_dot = ((fFx * sin(delta) + fFy * cos(delta) + fRy) /
                      m_Vehicle_m - vx * wz)
            if nn:
                pass
            ...
            if throttle_nn:
                ...
                next_state[:, 4] = wR + deltaT * \
                    throttle_nn(input_tensor).detach().numpy().flatten()
            else:
                # wR + deltaT * (m_Vehicle_kTorque * (T-wR) - m_Vehicle_rR * fRx) / m_Vehicle_IwR
                next_state[:, 4] = T
            rho = self.get_curvature(Y)
            next_state[:, 5] = psi + deltaT * \
                (wz - (vx * cos(psi) - vy * sin(psi)) / (1 - rho * X) * rho)
            next_state[:, 6] = X + deltaT * (vx * sin(psi) + vy * cos(psi))
            next_state[:, 7] = Y + deltaT * \
                (vx * cos(psi) - vy * sin(psi)) / (1 - rho * X)

            if len(cartesian) > 0:
                cartesian[0, :] += deltaT * wz
                cartesian[1, :] += deltaT * \
                    (cos(cartesian[0, :]) * vx - sin(cartesian[0, :]) * vy)
                cartesian[2, :] += deltaT * \
                    (sin(cartesian[0, :]) * vx + cos(cartesian[0, :]) * vy)
    ...
    def linearize_dynamics(self, states, controls):
        ...
        fx_plus = self.update_dynamics(
            x_plus, ux, dt, throttle_nn=self.throttle)
        # print(fx_plus)
        fx_minus = self.update_dynamics(
            x_minus, ux, dt, throttle_nn=self.throttle)
        ...
        fu_plus = self.update_dynamics(
            xu, u_plus, dt, throttle_nn=self.throttle)
        # print(fu_plus)
        fu_minus = self.update_dynamics(
            xu, u_minus, dt, throttle_nn=self.throttle)
        ...
        d = self.update_dynamics(
            states, controls, dt, throttle_nn=self.throttle) - np.dot(A, state_row) - np.dot(B, input_row)
    ...
    def form_long_matrices_LTV(self, A, B, d, D):
        ...
        for ii in np.arange(0, N):
            ...
            for jj in np.arange(ii-1, -1, -1):
                B_i_cell = np.dot(
                    A[:, :, ii], BB[(ii-1)*nx:ii*nx, jj*nu:(jj+1)*nu])
                B_i_row = np.hstack((B_i_cell, B_i_row))
                D_i_cell = np.dot(
                    A[:, :, ii], DD[(ii-1)*nx:ii*nx, jj*nl:(jj+1)*nl])
...
def run_simple_controller():
    ...
    ar = Model(N)
    x_target = np.tile(
        np.array([7, 0, 0, 0, 0, 0, 0, 0]).reshape((-1, 1)), (N, 1))
    ...
    try:
        for ii in range(int(sim_length/1)):
            ...
            solver.populate_params(
                A, B, d, D, xs[:, 0], sigma_0, sigma_N_inv, Q_bar, R_bar, us[:, 0], x_target)
            # A = np.eye(8)
            # A[0, 0] = 0.5
            # A[0, 4] = 0.5
            # A[5, 2] = 1
            ...
            try:
                ...
                print('RuntimeError')
                V = np.tile(
                    np.array([0, -1]).reshape((-1, 1)), (N, 1)).flatten()
                K = np.zeros((m*N, n*N))
            # ks[:, :, ii] = K[:, :]
            # ss[0, ii] = state[7, 0]
            # nearest = np.argmin(np.abs(state[7, 0] - ss[0, :]))
            ...
            for jj in range(1):
                # print(y)
                u = V[jj*m:(jj+1)*m] + \
                    np.dot(K[jj*m:(jj+1)*m, jj*n:(jj+1)*n], y)
                ...
                # print(state)
                # print(u)
                state, cartesian = ar.update_dynamics(state, u.reshape(
                    (-1, 1)), 0.1, throttle_nn=ar.throttle, cartesian=cartesian)
