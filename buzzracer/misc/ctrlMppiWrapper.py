...
class ctrlMppiWrapper(car):
    ...
    def init(self, track, sim=None):
        ...
        self.noise_cov = np.diag(
            [(self.max_throttle/2)**2, radians(40.0/2)**2])
...
# debug: a dictionary of objects to be debugged, e.g. {offset, error in v}
    ...
    def ctrl_car(self, state, track, v_override=None, reverse=False):
        ...
        p.s('mppi')
        uu = self.mppi.control(
            state.copy(), self.opponent_prediction, self.control_limit)
        control = uu[0]
        throttle = control[0]
        steering = control[1]
        p.e('mppi')
    ...
    def evaluate_step_cost(self, state, control):
        ...
        cost = np.sqrt((state[0]-self.raceline_points[0, ids])
                       ** 2+(state[2]-self.raceline_points[1, ids])**2) * 0.5
    ...
    def evaluate_terminal_cost(self, state, x0):
        ...
        cost_approx = -((ids - ids0 + self.discretized_raceline_len) %
                        self.discretized_raceline_len)*0.01
    ...
    def apply_discrete_dynamics(self, state, control, dt):
        ...
        if (vx < 0.05):
            ...
            Ffy = Df * np.sin(C * np.arctan(B * slip_f)) * \
                9.8 * lr / (lr + lf) * m
            Fry = Dr * np.sin(C * np.arctan(B * slip_r)) * \
                9.8 * lf / (lr + lf) * m
