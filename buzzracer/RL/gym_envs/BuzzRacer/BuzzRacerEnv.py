...
class BuzzRacerEnv(gym.Env):
    ...
    def draw_control(self, canvas, coord=(0, 0)):
        ...
        if (steering > 0):
            ...
            pygame.draw.rect(canvas, (0, 200, 0),
                             (x1+w/2, y1, length, h), width=0)
            print(length)
        else:
            length = fmap(-steering, 0, radians(26.7), 0, w/2)
            pygame.draw.rect(canvas, (0, 200, 0),
                             (x1+w/2-length, y1, length, h), width=0)
            print(length)

        if (throttle > 0):
            length = fmap(throttle, 0, 1, 0, w/2)
            pygame.draw.rect(canvas, (0, 200, 0),
                             (x2+w/2, y2, length, h), width=0)
            print(length)
        else:
            length = fmap(-throttle, 0, 1, 0, w/2)
            pygame.draw.rect(canvas, (0, 200, 0),
                             (x2+w/2-length, y2, length, h), width=0)
            print(length)
        ...
        y1 = coord[1]
        # Add steering bar
        img = cv2.rectangle(img, (x1 + 4, y1 + 25),
        ...
        # Add Throttle bar
        img = cv2.rectangle(img, (x1 + 4, y1 + 45),
