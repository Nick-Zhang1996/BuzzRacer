...
rl_dir = os.path.dirname(os.path.dirname(
...
class BuzzRacerEnvDiscrete(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.observation_space = spaces.Box(low=np.array([0.0, -0.3, -radians(180), -0.05, -2.0, -np.pi*2]), high=np.array([
                                            11.5, 0.3, radians(180), 5.0, 2.0, np.pi*2]), dtype=np.float32)
        # throttle, steering
        # self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(2,), dtype=np.float32)
        self.ds = 5
        self.action_space = spaces.Discrete(self.ds*self.ds)
        self.observation_space.n = 6
        self.action_space.n = self.ds*self.ds
    ...
    def init_visualization(self):
        ...
        self.screen = pygame.display.set_mode(
            self.background.get_size(), pygame.SCALED)
        ...
        self.car_image = pygame.transform.scale(
            self.car_image, (scale*ori_size[0], scale*ori_size[1]))
    ...
    def step(self, discrete_action):
        ...
        throttle = (int(discrete_action)//self.ds)/(self.ds-1)
        steering = (int(discrete_action) % self.ds)/(self.ds-1)
        throttle = (throttle*2)-1.0
        steering = (steering*2)-1.0
