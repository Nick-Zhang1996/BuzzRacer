# Game imports
import gc
from car_racing.orca_env_function import getfreeze_timecollosion_reachedreward
...
for t_eps in range(num_episode):
    ...
    while np.all(done.numpy()) == False:
        ...
        st1_gpu = torch.cat(
            [state_c1[:, 0:5], state_c2[:, 0:5]], dim=1).to(device)

        dist1 = p1(st1_gpu)
        action1 = dist1.sample().to('cpu')

        st2_gpu = torch.cat(
            [state_c2[:, 0:5], state_c1[:, 0:5]], dim=1).to(device)
        ...
        if itr > 0:
            ...
            mat_state2 = torch.cat([mat_state2.view(-1, curr_batch_size, 5),
                                   state_c2[:, 0:5].view(-1, curr_batch_size, 5)], dim=0)
            mat_action1 = torch.cat(
                [mat_action1.view(-1, curr_batch_size, 2), action1.view(-1, curr_batch_size, 2)], dim=0)
            mat_action2 = torch.cat(
                [mat_action2.view(-1, curr_batch_size, 2), action2.view(-1, curr_batch_size, 2)], dim=0)
        else:
        ...
        if curr_batch_size < remaining_xo.size(0):
            if batch_mat_action1.nelement() == 0:
                ...
                batch_mat_state2 = mat_state2.transpose(
                    0, 1)[~remaining_xo].view(-1, 5)
                batch_mat_action1 = mat_action1.transpose(
                    0, 1)[~remaining_xo].view(-1, 2)
                batch_mat_action2 = mat_action2.transpose(
                    0, 1)[~remaining_xo].view(-1, 2)
                ...
                progress_done2 = torch.sum(mat_state2.transpose(0, 1)[~remaining_xo][:, mat_state2.size(
                    0)-1, 0] - mat_state2.transpose(0, 1)[~remaining_xo][:, 0, 0])
                ...
                batch_mat_state2 = torch.cat([batch_mat_state2, mat_state2.transpose(0, 1)[
                                             ~remaining_xo].view(-1, 5)], dim=0)
                batch_mat_action1 = torch.cat([batch_mat_action1, mat_action1.transpose(0, 1)[
                                              ~remaining_xo].view(-1, 2)], dim=0)
                batch_mat_action2 = torch.cat([batch_mat_action2, mat_action2.transpose(0, 1)[
                                              ~remaining_xo].view(-1, 2)], dim=0)
            ...
            mat_state2 = mat_state2.transpose(
                0, 1)[remaining_xo].transpose(0, 1)
            mat_action1 = mat_action1.transpose(
                0, 1)[remaining_xo].transpose(0, 1)
            mat_action2 = mat_action2.transpose(
                0, 1)[remaining_xo].transpose(0, 1)
        ...
        if np.all(done.numpy()) == True or batch_mat_state1.size(0) > 900 or itr > 400:
            prev_size = batch_mat_state1.size(0)
    ...
    returns_np1 = get_advantage(
        next_value, batch_mat_reward1, val1, batch_mat_done, gamma=0.99, tau=0.95)
