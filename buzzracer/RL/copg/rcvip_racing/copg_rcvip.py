...
try:
    last_checkpoint_eps = torch.load(os.path.join(
        folder_location, experiment_name, f'last_checkpoint_eps.pth'))
    print(
        f'resuming training from {last_checkpoint_eps}, optimizer state not loaded')
    t_eps = last_checkpoint_eps
    p1_state_dict = torch.load(os.path.join(
        folder_location, experiment_name, 'model', f'agent1_{t_eps}.pth'))
    p2_state_dict = torch.load(os.path.join(
        folder_location, experiment_name, 'model', f'agent2_{t_eps}.pth'))
    q_state_dict = torch.load(os.path.join(
        folder_location, experiment_name, 'model', f'val_{t_eps}.pth'))
...
def simulate(device, t):
    ...
    state_c1 = torch.zeros(batch_size, n_state).to(
        device)  # state[:,0:6].view(6)
    state_c2 = torch.zeros(batch_size, n_state).to(
        device)  # state[:, 6:12].view(6)
    init_p1 = torch.zeros((batch_size)).to(
        device)  # 5*torch.rand((batch_size))
    init_p2 = torch.zeros((batch_size)).to(
        device)  # 5*torch.rand((batch_size))
    state_c1[:, 0] = init_p1
    state_c2[:, 0] = init_p2
    ...
    while np.all(done.cpu().numpy()) == False:
        ...
        t.s('cat state')
        # concate along dim = 0
        mat_state1 = torch.cat(
            [mat_state1,   state_c1[:, 0:5].view(-1, batch_size, 5)], dim=0)
        mat_state2 = torch.cat(
            [mat_state2,   state_c2[:, 0:5].view(-1, batch_size, 5)], dim=0)
        mat_action1 = torch.cat(
            [mat_action1, action1.view(-1, batch_size, 2)], dim=0)
        mat_action2 = torch.cat(
            [mat_action2, action2.view(-1, batch_size, 2)], dim=0)
        t.e('cat state')
        ...
        mat_reward1 = torch.cat(
            [mat_reward1.view(-1, batch_size, 1), reward1.view(-1, batch_size, 1)], dim=0)
        mat_done = torch.cat(
            [mat_done.view(-1, batch_size, 1), ~done.view(-1, batch_size, 1)], dim=0)
        ...
        if batch_size < done.size(0):
            if batch_mat_action1.nelement() == 0:
                ...
                batch_mat_action1 = mat_action1.transpose(0, 1)[
                    done].view(-1, 2)
                batch_mat_action2 = mat_action2.transpose(0, 1)[
                    done].view(-1, 2)
                batch_mat_reward1 = mat_reward1.transpose(0, 1)[
                    done].view(-1, 1)
                ...
                progress_done1 = torch.sum(mat_state1.transpose(0, 1)[done][:, mat_state1.size(
                    0)-1, 0] - mat_state1.transpose(0, 1)[done][:, 0, 0])
                progress_done2 = torch.sum(mat_state2.transpose(0, 1)[done][:, mat_state2.size(
                    0)-1, 0] - mat_state2.transpose(0, 1)[done][:, 0, 0])
                ...
                batch_mat_state1 = torch.cat(
                    [batch_mat_state1, mat_state1.transpose(0, 1)[done].view(-1, 5)], dim=0)
                batch_mat_state2 = torch.cat(
                    [batch_mat_state2, mat_state2.transpose(0, 1)[done].view(-1, 5)], dim=0)
                batch_mat_action1 = torch.cat(
                    [batch_mat_action1, mat_action1.transpose(0, 1)[done].view(-1, 2)], dim=0)
                batch_mat_action2 = torch.cat(
                    [batch_mat_action2, mat_action2.transpose(0, 1)[done].view(-1, 2)], dim=0)
                batch_mat_reward1 = torch.cat(
                    [batch_mat_reward1, mat_reward1.transpose(0, 1)[done].view(-1, 1)], dim=0)
                batch_mat_done = torch.cat(
                    [batch_mat_done, mat_done.transpose(0, 1)[done].view(-1, 1)], dim=0)
        ...
        if np.all(done.cpu().numpy()) == True or batch_mat_state1.size(0) > 900 or itr > 400:
            prev_size = batch_mat_state1.size(0)
            batch_mat_state1 = torch.cat(
                [batch_mat_state1, mat_state1.transpose(0, 1).reshape(-1, 5)], dim=0)
            batch_mat_state2 = torch.cat(
                [batch_mat_state2, mat_state2.transpose(0, 1).reshape(-1, 5)], dim=0)
            batch_mat_action1 = torch.cat(
                [batch_mat_action1, mat_action1.transpose(0, 1).reshape(-1, 2)], dim=0)
            batch_mat_action2 = torch.cat(
                [batch_mat_action2, mat_action2.transpose(0, 1).reshape(-1, 2)], dim=0)
...
def update(batch_mat_state1, batch_mat_action1, batch_mat_reward1, batch_mat_state2, batch_mat_action2, batch_mat_done, device, t):
    ...
    returns_np1 = get_advantage(next_value, batch_mat_reward1,
                                val1, batch_mat_done, gamma=0.99, tau=0.95, device=device)
    ...
    state_gpu_p1 = torch.cat(
        [batch_mat_state1, batch_mat_state2], dim=1).to(device)
    state_gpu_p2 = torch.cat(
        [batch_mat_state2, batch_mat_state1], dim=1).to(device)
    ...
    # otherwise it doesn't change values
    ...
    for i in range(1, log_probs1.size(0)):
        s_log_probs1[i] = torch.add(
            s_log_probs1[i - 1], log_probs1[i-1])*mask[i-1]
        s_log_probs2[i] = torch.add(
            s_log_probs2[i - 1], log_probs2[i-1])*mask[i-1]

    objective2 = s_log_probs1[1:s_log_probs1.size(
        0)]*log_probs2[1:log_probs2.size(0)]*(val1_p[0, 1:val1_p.size(1)])
    ob2 = objective2.sum()/(objective2.size(0)-batch_size+1)

    objective3 = log_probs1[1:log_probs1.size(
        0)]*s_log_probs2[1:s_log_probs2.size(0)]*(val1_p[0, 1:val1_p.size(1)])
    ...
    writer.add_scalar('Entropy/agent1',
                      dist_batch1.entropy().mean().detach(), t_eps)
    writer.add_scalar('Entropy/agent2',
                      dist_batch2.entropy().mean().detach(), t_eps)
...
for t_eps in range(last_checkpoint_eps, num_episode):
    ...
    t.s('update')
    update(batch_mat_state1, batch_mat_action1, batch_mat_reward1,
           batch_mat_state2, batch_mat_action2, batch_mat_done, device, t)
    t.e('update')
    ...
    if t_eps % 20 == 0:
        torch.save(p1.state_dict(), os.path.join(folder_location,
                   experiment_name, 'model', f'agent1_{t_eps}.pth'))
        torch.save(p2.state_dict(), os.path.join(folder_location,
                   experiment_name, 'model', f'agent2_{t_eps}.pth'))
        torch.save(q.state_dict(), os.path.join(folder_location,
                   experiment_name, 'model', f'val_{t_eps}.pth'))
        torch.save(t_eps, os.path.join(folder_location,
                   experiment_name, f'last_checkpoint_eps.pth'))
    t.e()
    sys.stdout.flush()
t.summary()
