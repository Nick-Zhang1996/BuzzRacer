...
class CCMPPI:
    def __init__(self, arg_list):
        ...
        print_info('registers used each kernel in eval_ctrl= %d' %
                   self.cuda_evaluate_control_sequence.num_regs)
        print('total registers used in one block (%d/%d)(used/available)' %
              (int(self.cuda_evaluate_control_sequence.num_regs * self.cuda_block_size[0]), 65536))
        assert int(self.cuda_evaluate_control_sequence.num_regs *
                   self.cuda_block_size[0]) <= 65536
        assert int(self.cuda_init_curand_kernel.num_regs *
                   self.cuda_block_size[0]) <= 65536
        assert int(self.cuda_generate_random_var.num_regs *
                   self.cuda_block_size[0]) <= 65536
        ...
        self.device_discretized_raceline = drv.to_device(
            self.discretized_raceline)
    ...
    def control(self, state, opponents_prediction, control_limit):
        ...
        ref_control = np.vstack(
            [self.old_ref_control[1:, :], np.zeros([1, self.m], dtype=np.float32)])
        ...
        scales = np.array([np.sqrt(noise_cov[0, 0]), np.sqrt(
            noise_cov[1, 1])], dtype=np.float32)
        device_scales = drv.to_device(scales)

        self.cuda_generate_random_var(self.device_rand_vals, device_scales, block=(
            self.curand_kernel_n, 1, 1), grid=(1, 1, 1))
        # DEBUG
        self.rand_vals = drv.from_device(self.device_rand_vals, shape=(
            self.K*self.T*self.m,), dtype=np.float32)
        ...
        # opponent position
        opponents_prediction = np.array(
            opponents_prediction).astype(np.float32)
        ...
        memCount = cost.size*cost.itemsize + control.size*control.itemsize + x0.size*x0.itemsize + ref_control.size*ref_control.itemsize + self.rand_vals.size*self.rand_vals.itemsize + self.rand_vals.size * \
            self.rand_vals.itemsize + self.discretized_raceline.flatten().size*self.rand_vals.flatten().itemsize + \
            Ks_flat.size*Ks_flat.itemsize + As_flat.size * \
            As_flat.itemsize + Bs_flat.size*Bs_flat.itemsize
