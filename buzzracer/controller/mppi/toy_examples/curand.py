...
cuda_code_macros = {'SAMPLE_COUNT': K, 'HORIZON': T, 'CONTROL_DIM': m,
                    'STATE_DIM': state_dim, 'RACELINE_LEN': 1024, 'TEMPERATURE': temperature, 'DT': dt}
...
tic = time()
for i in range(100):
    cuda_generate_random_var(device_rand_vals, device_scales,
                             device_limits, block=(1024, 1, 1), grid=(1, 1, 1))
    # TODO make sure rand_vals is usable by device
    rand_vals = drv.from_device(
        device_rand_vals, shape=(K*T*m,), dtype=np.float32)
tac = time()
...
tic = time()
for i in range(100):
    cuda_generate_random_var(
        device_rand_vals, device_limits, block=(1024, 1, 1), grid=(1, 1, 1))
    # TODO make sure rand_vals is usable by device
