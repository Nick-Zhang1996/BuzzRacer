...
class MPC:
    ...
    def convert_ltv(self, A_vec, B_vec, C, P, Q, y_ref, x0, du_max, u_max):
        ...
        h4 = np.array(du_max).flatten() + \
            np.array(self.last_applied_u).flatten()
        # print(self.last_applied_u)

        G5 = np.zeros([m, p*m])
        G5 = np.block([-np.eye(m), np.zeros([m, (p-1)*m])])
        h5 = np.array(du_max).flatten() - \
            np.array(self.last_applied_u).flatten()
        # print(h5)
        # print('--')
    ...
    # P is for a single state vector, it will be broadcasted automatically
    # NOTE currently for single input only
    def convert_lti_planner(self, A, B, P, Q, xref, x0, p, u_max=None, du_max=None):
        p = self.p
        n = self.n
        m = self.m
        l = self.l
    ...
    def solve(self):
        ...
        self.last_u = sol['x']
        self.cost = sol['primal objective']
