...
if __name__ == '__main__':
    ...
    textlog_filename = os.path.join(os.path.dirname(sys.argv[0]),'../../log',name,'textlog9b.txt')
    with open(textlog_filename,'r') as f:
        lines = f.readlines()

    # parse log file
    labels = ['experiment_name' , 'config_file_name' , 'log_name' , 'laps' , 'enable_cvar','cvar_A' , 'cvar_a' , 'cvar_Cu' , 'laptime_mean' , 'laptime_stddev' , 'boundary_violation' , 'obstacle_violation','noise'] 
    data_dict = {}
    for label in labels:
        data_dict[label] = []
    ...
    # get noise
    cvar_noise_data = np.array(data_dict['noise'])
    ...
    cvar_noise_vec = [0.3,0.5,0.7]

    grid_cvar = np.zeros((len(cvar_a_vec), len(cvar_noise_vec)))
    #grid_baseline = np.zeros((len(cvar_a_vec), len(cvar_noise_vec)))
    for i in range(len(cvar_a_vec)):
        for j in range(len(cvar_noise_vec)):
            cvar_a = cvar_a_vec[i]
            cvar_noise = cvar_noise_vec[j]
            mask1 = np.isclose(cvar_a_data,cvar_a)
            mask2 = np.isclose(cvar_noise_data,cvar_noise)
    ...
    advantage = pandas.DataFrame(data=advantage, index=cvar_a_vec, columns=cvar_noise_vec)
