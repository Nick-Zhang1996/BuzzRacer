...
'''
# v_forward,v_sideway,omega
train_x = torch.stack([
    torch.rand(training_pts)*0.1,
    torch.rand(training_pts)*0.1,
    torch.rand(training_pts)*0.01,
    torch.rand(training_pts)*2-1,
    (torch.rand(training_pts)*2-1)*math.radians(26.1)
    ],-1)
train_y = kinematics(train_x)
...
'''
test_x = torch.stack([torch.rand(training_pts), torch.rand(training_pts)],-1)
test_y = torch.stack([
    torch.sin((2 * math.pi)*train_x[:,0]),
    -2.5*torch.cos((2 * math.pi)*train_x[:,1]**2)*torch.exp(-2*train_x[:,0]),
    ],-1)
