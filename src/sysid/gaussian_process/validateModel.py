...
sys.path.append(os.path.dirname(os.path.dirname(thisdir)))
...
def step_raw(state, control, dt=0.01, slip_f_override=None):
    ...
    debug_dict = {'slip_f': slip_f, 'slip_r': slip_r, 'lateral_acc_f': Ffy /
                  m, 'lateral_acc_r': Fry/m, 'ax': d_vx, 'dw': d_omega, 'w': omega}
...
def run():
    ...
    debug_dict_hist = {'slip_f': [[]], 'slip_r': [[]], 'lateral_acc_f': [
        []], 'lateral_acc_r': [[]], 'ax': [[]], 'dw': [[]], 'w': [[]]}
    ...
    for i in range(1, data_len-lookahead_steps-1):
        ...
        # velocity predicted
        v_predicted_hist = (
            predicted_states[:, 1]**2 + predicted_states[:, 3]**2)**0.5
        vx_car_predicted_hist = predicted_states[:, 1]

        # forward
        vx_car_predicted_hist = predicted_states[:, 1]*np.cos(
            predicted_states[:, 4]) + predicted_states[:, 3]*np.sin(predicted_states[:, 4])
        # lateral, left +
        vy_car_predicted_hist = -predicted_states[:, 1]*np.sin(
            predicted_states[:, 4]) + predicted_states[:, 3]*np.cos(predicted_states[:, 4])
        ...
        if (False and i % 100 == 0):
            ...
            ax1 = plt.subplot(312)
            ax1.plot(
                np.array(debug_dict_hist['w'][i])/np.pi*180, label='omega predicted')
...
if __name__ == '__main__':
    ...
    if saveGif:
        ...
        gifs[0].save(fp=gif_filename, format='GIF',
                     append_images=gifs, save_all=True, duration=20, loop=0)
