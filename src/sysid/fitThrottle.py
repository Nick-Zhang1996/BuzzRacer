# study longitudinal acceleration model
# analyze log
import pickle
...
def getAccCarFrame(x,y,dt):
    ...
    for i in range(vel_vec.shape[0]-1):
        ...
        # TODO this does not distinguish between lateral and logitudinal speed
...
ax.plot(t[:-2],lon_acc, label="longitudinal acc measured")
ax.plot(t,throttle, label="throttle")
#ax.plot(t,6*(throttle-0.24), label="heuristic")
ax.plot(t,8*(throttle-0.27)-np.abs(steering)*0, label="heuristic")
#ax.plot(t,steering*5, label="steering")
ax.hlines(0,t[0],t[-1])
ax.legend()
plt.show()
