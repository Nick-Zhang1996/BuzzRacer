...
class RecordGif(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        self.truncated = False
        if self.recording:
            assert self.gif_recorder is not None
    ...
    def render(self, *args, **kwargs):
        ...
        if len(self.gif_recorder.render_history) > 0:
            recorded_frames = [
                self.gif_recorder.render_history.pop()
                for _ in range(len(self.gif_recorder.render_history))
            ]
            if self.recording:
