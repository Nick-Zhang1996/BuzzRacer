...
def simulate(device):
    ...
    for i in range(2000):
        ...
        mat_action = torch.cat(
            [mat_action, action.view(-1, 1, n_action)], dim=1)
        ...
        if (np.any(done.numpy())):
            ...
            state_vec = torch.cat(
                [state_vec, mat_state[done].view(-1, n_state)])
            action_vec = torch.cat(
                [action_vec, mat_action[done].view(-1, n_action)])
...
for t_eps in range(last_checkpoint_eps, num_episode):
    ...
    update(batch_mat_state1, batch_mat_action1,
           batch_mat_reward1, batch_mat_done, device)
