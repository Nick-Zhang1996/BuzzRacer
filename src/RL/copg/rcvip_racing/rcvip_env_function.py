...
def getfreezeTimecollosionReachedreward(state_c1, state_c2, lb_c1, lb_c2, prev_state_c1, prev_state_c2, prev_coll_c1, prev_coll_c2, counter1, counter2, device):
    ...
    counter1 = counter1 + prev_coll_c1 * \
        torch.ones(reward.shape, device=device)  # increasing counter if collided
    # if counter is above 34 then reseting
    prev_coll_c1 = (counter1 < 34) * prev_coll_c1
    counter1 = counter1*(counter1 < 34)  # this will reset the counter

    counter2 = counter2 + prev_coll_c2 * \
        torch.ones(reward.shape, device=device)  # increasing counter if collided
    # if counter is above 34 then reseting
    prev_coll_c2 = (counter2 < 34) * prev_coll_c2
    counter2 = counter2*(counter2 < 34)  # this will reset the counter

    # print(counter1, counter2)
    # if it collided earlier it should freeze
    c1_coll = ~((~c1_coll) * (~prev_coll_c1))
    # if it collided earlier it should freeze
    c2_coll = ~((~c2_coll) * (~prev_coll_c2))
