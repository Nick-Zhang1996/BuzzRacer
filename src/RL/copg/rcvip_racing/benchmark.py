...
vehicle_model = VehicleModel.VehicleModel(
    config['n_batch'], device, config, track=track)
...
for i in range(duration):
    ...
    leading_player = torch.ones(state_c1.size(0)) * \
        ((state_c1[:, 0]-state_c2[:, 0]) > 0)
    ...
    if curr_batch_size < remaining_xo.size(0):
        ...
        # if t==1:
        #     print(i)
        a_win = a_win + torch.sum(torch.ones(out_state_c1.size(0))
                                  * (out_state_c1[:, 0] > out_state_c2[:, 0]))
        b_win = b_win + torch.sum(torch.ones(out_state_c1.size(0))
                                  * (out_state_c1[:, 0] < out_state_c2[:, 0]))
    ...
    if np.all(done.numpy()) == True or i == 1999:
        ...
        print('Normalized score for races between ' +
              player1 + ' vs ' + player2,)
        print('Overtakes per lap', np.array(
            torch.sum(torch.stack(over_mat))/init_size))
