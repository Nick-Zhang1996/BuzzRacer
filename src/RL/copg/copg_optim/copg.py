...
# Need for zero grad and conjugate gradient functions
from copg_optim.utils import zero_grad, conjugate_gradient, general_conjugate_gradient
...
class CoPG(object):
    ...
    def step(self, ob_tot, lp1, lp2):
        # can remove create graph
        grad_x = autograd.grad(lp1, self.max_params,
                               create_graph=True, retain_graph=True)
        grad_x_vec = torch.cat([g.contiguous().view(-1, 1) for g in grad_x])
        grad_y = autograd.grad(lp2, self.min_params,
                               create_graph=True, retain_graph=True)
        grad_y_vec = torch.cat([g.contiguous().view(-1, 1) for g in grad_y])
        tot_grad_y = autograd.grad(
            ob_tot.mean(), self.min_params, create_graph=True, retain_graph=True)
        tot_grad_y = torch.cat([g.contiguous().view(-1, 1)
                               for g in tot_grad_y])

        tot_grad_xy = autograd.grad(
            tot_grad_y, self.max_params, grad_outputs=grad_y_vec, retain_graph=True)
        hvp_x_vec = torch.cat([g.contiguous().view(-1, 1)
                              for g in tot_grad_xy])  # tot_xy

        tot_grad_x = autograd.grad(
            ob_tot.mean(), self.max_params, create_graph=True, retain_graph=True)
        tot_grad_x = torch.cat([g.contiguous().view(-1, 1)
                               for g in tot_grad_x])

        tot_grad_yx = autograd.grad(
            tot_grad_x, self.min_params, grad_outputs=grad_x_vec, retain_graph=True)
        hvp_y_vec = torch.cat([g.contiguous().view(-1, 1)
                              for g in tot_grad_yx])
        ...
        if self.solve_x:
            ...
            hcg = autograd.grad(tot_grad_y, self.max_params,
                                grad_outputs=cg_y, retain_graph=False)  # yx
            ...
            cg_x, self.iter_num = conjugate_gradient(grad_x=grad_x_vec, grad_y=grad_y_vec,
                                                     ...
                                                     nsteps=p_x.shape[0],
                                                     lr=self.lr, device=self.device)
            hcg = autograd.grad(tot_grad_x, self.min_params,
                                grad_outputs=cg_x, retain_graph=False)  # yx
            hcg = torch.cat([g.contiguous().view(-1, 1) for g in hcg])
        ...
        for p in self.max_params:
            ...
            p.data.add_(
                self.lr * cg_x[index: index + p.numel()].reshape(p.shape))
            index += p.numel()
        if index != cg_x.numel():
...
class RCoPG(object):
    ...
    def step(self, ob, ob_tot, lp1, lp2):
        ...
        self.square_avgx.mul_(self.beta2).addcmul_(
            grad_x_vec.data, grad_x_vec.data, value=1-self.beta2)
        self.square_avgy.mul_(self.beta2).addcmul_(
            grad_y_vec.data, grad_y_vec.data, value=1-self.beta2)
        ...
        lr_x = math.sqrt(bias_correction2) * self.lr / \
            self.square_avgx.sqrt().add(self.eps)
        lr_y = math.sqrt(bias_correction2) * self.lr / \
        ...
        tot_grad_xy = autograd.grad(
            tot_grad_y, self.max_params, grad_outputs=scaled_grad_y, retain_graph=True)
        hvp_x_vec = torch.cat([g.contiguous().view(-1, 1)
                              for g in tot_grad_xy])  # D_xy * lr_y * grad_y
        ...
        tot_grad_yx = autograd.grad(
            tot_grad_x, self.min_params, grad_outputs=scaled_grad_x, retain_graph=True)
        hvp_y_vec = torch.cat([g.contiguous().view(-1, 1)
                              for g in tot_grad_yx])  # D_yx * lr_x * grad_x)

        # grad_x - D_xy * lr_y * grad_y
        p_x = torch.add(grad_x_vec, - hvp_x_vec).detach_()
        # grad_y + D_yx * lr_x * grad_x
        p_y = torch.add(grad_y_vec, hvp_y_vec).detach_()
