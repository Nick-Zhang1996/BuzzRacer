...
class RCPTrack(Track):
    ...
    def initTrack(self, description, gridsize, scale, savepath=None):
        ...
        lookup_table = {'WE': ['rr', 'll'], 'NS': ['uu', 'dd'], 'SE': ['ur', 'ld'], 'SW': ['ul', 'rd'],
                        'NE': ['dr', 'lu'], 'NW': ['ru', 'dl']}
    ...
    def initRaceline(self, start, start_direction, seq_no, offset=None, filename=None):
        ...
        tck, u = splprep(pts.T, u=np.linspace(0, self.track_length_grid, self.track_length_grid + 1),
                         s=smoothing_factor, per=1)
        # NOTE
    ...
    def generateSpeedProfile(self, n_steps=1000):
        ...
        for i in np.linspace(min_xx, min_xx - n_steps, n_steps + 2):
            ...
            ds = dist((x_i, y_i), (x_i_1, y_i_1))
            # print(ds)
            v3[(i - 1 + n_steps) % n_steps] = min((v3[i % n_steps] ** 2 + 2 * a_lon * ds) ** 0.5,
                                                  v2[(i - 1 + n_steps) % n_steps])
    ...
    def simulateSensors(self, car, track, delta=.0001, ):
        # print(car)
        states = car.states
    ...
    def simulateSensorsOnlyNums(self, state, delta=.001, ):
        ...
        while self.checkTrackBoundary((x, y)) > 0:
            x += i * np.cos(state[2] + np.pi / 2)
            y += i * np.sin(state[2] + np.pi / 2)
            i += delta
        right2 = i
        ...
        i = delta
        while self.checkTrackBoundary((x, y)) > 0:
            x += i * np.cos(state[2] - np.pi)
            y += i * np.sin(state[2] - np.pi)
            i += delta
        behind = i

        return np.array([forward, right2, behind, left2])
    ...
    def minimizeCurvatureRoutine(self, ):
        ...
        self.cost_count = 0
        res = minimize(self.cost, K0, method='SLSQP', jac='2-point', constraints=cons, bounds=bnds,
                       options={'maxiter': 1000, 'eps': 1e-10})
        print(res)
        # verify again
        self.K = res.x
        print(self.K)
    ...
    def drawRaceline(self, lineColor=(0, 0, 255), img=None, points=None):
        ...
        for i in range(len(u_new) - 1):
            img = cv2.line(img, tuple(pts[i]), tuple(pts[i + 1]),
                           color=getColor(self.targetVfromU(u_new[i] % self.track_length_grid)), thickness=3)
    ...
    def localTrajectory(self, state, wheelbase=90e-3, return_u=False):
        ...
        if return_u:
            return (raceline_point, copysign(abs(min_fun_val) ** 0.5, cross_theta), atan2(der[1], der[0]),
                    copysign(norm_curvature, cross_curvature), request_velocity, min_fun_x % self.track_length_grid)
        else:
            return (raceline_point, copysign(abs(min_fun_val) ** 0.5, cross_theta), atan2(der[1], der[0]),
                    copysign(norm_curvature, cross_curvature), request_velocity)
