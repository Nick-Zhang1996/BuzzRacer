...
class PerformanceTracker(Extension):
    ...
    def final(self):
        pos_2_norm = np.mean(self.car.controller.pos_2_norm_vec)
        print_ok(self.prefix()+"Position 2 norm = %.6f"%(pos_2_norm))

        state_2_norm = np.mean(self.car.controller.state_2_norm_vec)
        print_ok(self.prefix()+"state 2 norm = %.6f"%(state_2_norm))

        pos_area = np.mean(self.car.controller.pos_area_vec)
