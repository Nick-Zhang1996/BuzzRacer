...
# https://github.com/Nick-Zhang1996/miniz-board/blob/main/BuzzRacerV2.py
from common import *
import socket
...
class BuzzRacerV2Packet(PrintObject):
    out_seq_no = 0
    packet_size = 64
    def __init__(self):
    ...
    def makePacket(self):
        self.seq_no = BuzzRacerV2Packet.out_seq_no
        ...
        padding_size = BuzzRacerV2Packet.packet_size - len(header) - len(self.payload)
        padding = pack('x'*padding_size)
        self.packet = header+self.payload+padding
        BuzzRacerV2Packet.out_seq_no += 1
...
class BuzzRacerV2(Car):
    ...
    def initSocket(self):
        ...
        self.local_port = BuzzRacerV2.available_local_port
        BuzzRacerV2.available_local_port += 1
    ...
    def __commThreadFunction( self, arg ):
        ...
        while not self.flag_quit.is_set():
            ...
            try:
                ...
                while True:
                    data, addr = self.sock.recvfrom( BuzzRacerV2Packet.packet_size ) # read 1 packet
                    if( len( data ) > 0 ):
                        assert len( data ) == BuzzRacerV2Packet.packet_size
    ...
    def parseResponse(self,data):
        ...
        if (packet_type == 2):
            ...
            self.lidar_front = packet.lidar[0]/100 + 0.04
            self.lidar_left = packet.lidar[1]/100 + 0.03
            self.lidar_rear = packet.lidar[2]/100 + 0.04
            self.lidar_right = packet.lidar[3]/100 + 0.03
    ...
    def prepareCommandPacket(self,throttle=0.0,steering=0.0):
        ...
        packet.src_addr = 0
        # FIXME temp
        steering = steering / (20 / 180.0 * np.pi) * 1.4
