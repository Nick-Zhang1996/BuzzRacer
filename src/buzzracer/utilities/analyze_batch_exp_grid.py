...
if __name__ == '__main__':
    ...
    textlog_filename = os.path.join(os.path.dirname(
        sys.argv[0]), '../../log', name, 'textlog.txt')
    with open(textlog_filename, 'r') as f:
        lines = f.readlines()

    # parse log file
    labels = ['experiment_name', 'config_file_name', 'log_name', 'laps', 'enable_cvar', 'cvar_A',
              'cvar_a', 'cvar_Cu', 'laptime_mean', 'laptime_stddev', 'boundary_violation', 'obstacle_violation']
    data_dict = {}
    for label in labels:
        data_dict[label] = []
    ...
    advantage = pandas.DataFrame(
        data=advantage, index=cvar_a_vec_text, columns=cvar_Cu_vec_text)
