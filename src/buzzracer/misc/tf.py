...
class TF:
    ...
    def euler2q(self, roll, pitch, yaw):
        q = np.array([cos(roll/2)*cos(pitch/2)*cos(yaw/2)+sin(roll/2)*sin(pitch/2)*sin(yaw/2), -cos(roll/2)*sin(pitch/2)*sin(yaw/2)+cos(pitch/2)*cos(yaw/2)*sin(roll/2),
                     cos(roll/2)*cos(yaw/2)*sin(pitch/2) + sin(roll/2)*cos(pitch/2)*sin(yaw/2), cos(roll/2)*cos(pitch/2)*sin(yaw/2) - sin(roll/2)*cos(yaw/2)*sin(pitch/2)])
        return q
    ...
    def euler2_r(self, roll, pitch, yaw):
        ...
        R = [[cos(pitch)*cos(yaw), cos(pitch)*sin(yaw), -sin(pitch)],
             [sin(roll)*sin(pitch)*sin(yaw)-cos(roll)*sin(yaw), sin(roll) *
              sin(pitch)*sin(yaw)+cos(roll)*cos(yaw), cos(pitch)*sin(roll)],
    ...
    def euler2_rxyz(self, roll, pitch, yaw):
        ...
        '''
        Rx = np.matrix([[1, 0, 0], [0, cos(roll), sin(roll)],
                       [0, -sin(roll), cos(roll)]])
