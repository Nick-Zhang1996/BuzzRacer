...
class CSSolver:
        ...
        try:
            ...
            if not mean_only:
                if lti_k:
                    ...
                    K = M.variable(
                        [m*N, n*N], Domain.sparse(Domain.unbounded(), pattern))
            else:
                pattern = []
                for ii in range(N):
                    for jj in range(m):
            ...
            if not mean_only:
                ...
                M.objective(ObjectiveSense.Minimize,
                            Expr.add([y1, y2, z1, z2]))
                M.constraint(Expr.vstack(0.5, w, Expr.mul(
                    Q_bar_half_B, V)), Domain.inRotatedQCone())
                M.constraint(Expr.vstack(0.5, x, Expr.mul(
                    R_bar_half, V)), Domain.inRotatedQCone())

                M.constraint(Expr.vstack(0.5, y1, Expr.flatten(Expr.mul(Q_bar_half, Expr.mul(
                    Expr.add(I, Expr.mul(B, K)), A_sigma_0_half)))), Domain.inRotatedQCone())
                # check BKD multiplicaion, or maybe with I? Something seems wrong here b/c sparsity pattern is invalid
                M.constraint(Expr.vstack(0.5, y2, Expr.flatten(Expr.mul(Q_bar_half, Expr.mul(
                    Expr.add(I, Expr.mul(B, K)), D)))), Domain.inRotatedQCone())
                M.constraint(Expr.vstack(0.5, z1, Expr.flatten(
                    Expr.mul(R_bar_half, Expr.mul(K, A_sigma_0_half)))), Domain.inRotatedQCone())
                M.constraint(Expr.vstack(0.5, z2, Expr.flatten(
                    Expr.mul(R_bar_half, Expr.mul(K, D)))), Domain.inRotatedQCone())
            else:
            ...
            M.constraint(Expr.sub(V.slice(2, N*m), V.slice(0, N*m-2)),
                         Domain.inRange(-0.2, 0.2))
            ...
            u_0.setValue(0.5)
            # print(u_0.getValue())
            M.constraint(Expr.sub(self.u_o, V.index(1)),
                         Domain.inRange(-0.2, 0.2))
            M.constraint(Expr.sub(self.u_s, V.index(0)),
                         Domain.inRange(-0.2, 0.2))
            ...
            mu_N = np.zeros((n, 1))
            # TODO
            mu_N = np.array([7.5, 2., 2.5, 100., 100., 0.5,
                            1.0, 1000.]).reshape((8, 1))
            ...
            E_N_T = Matrix.sparse(
                np.hstack((np.zeros((n, (N - 1) * n)), np.eye(n))).T)
            M.constraint(Expr.mul(E_N, Expr.add(
                Expr.add(A_mu_0, Expr.mul(B, V)), d)), Domain.lessThan(mu_N))
            e_n = -1 * e_n
            E_N = Matrix.sparse(np.hstack((np.zeros((n, (N - 1) * n)), e_n)))
            E_N_T = Matrix.sparse(
                np.hstack((np.zeros((n, (N - 1) * n)), np.eye(n))).T)
            ...
            for ii in range(N):
                ...
                e_k = np.eye(n)
                E_k = Matrix.sparse(
                    np.hstack((np.zeros((n, (ii) * n)), e_k, np.zeros((n, (N - ii - 1) * n)))))
                mean_part = Expr.mul(alpha_T, Expr.mul(
                    E_k, Expr.add(Expr.add(A_mu_0, Expr.mul(B, V)), d)))
                # if not mean_only:
                sigma_0_part = M.variable()
                M.constraint(Expr.vstack(sigma_0_part, Expr.flatten(Expr.mul(alpha_T, Expr.mul(
                    E_k, Expr.mul(Expr.add(I, Expr.mul(B, K)), A_sigma_0_half))))), Domain.inQCone())
                D_part = M.variable()
                M.constraint(Expr.vstack(D_part, Expr.flatten(Expr.mul(alpha_T, Expr.mul(
                    E_k, Expr.mul(Expr.add(I, Expr.mul(B, K)), D)))).slice(0, (ii+1)*l)), Domain.inQCone())
                cov_part = M.variable()
                M.constraint(Expr.vstack(
                    cov_part, sigma_0_part, D_part), Domain.inQCone())
                M.constraint(Expr.add(mean_part, Expr.mul(
                    cov_part, inv_prob)), Domain.inRange(-beta, beta))
    ...
    def populate_params(self, A, B, d, D, mu_0, sigma_0, sigma_N_inv, Q_bar, R_bar, u_0, x_target, K=None):
        ...
        self.mu_0_T_A_T_Q_bar_B.setValue(
            2*np.dot(np.dot(np.dot(mu_0.T, A.T), Q_bar), B))
    ...
    def solve(self):
        ...
        try:
            if self.mean_only:
                ...
                if self.lti_k:
                    K_level = np.kron(
                        np.eye(self.N), self.k.level().reshape((self.m, self.n)))
                else:
