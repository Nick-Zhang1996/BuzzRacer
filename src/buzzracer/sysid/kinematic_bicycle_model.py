...
class KinematicBicycleModelCartesian(VehicleDynamics):
    ...
    def advance_dynamics(state: CartesianState,
        ...
        del curvature
        if use_torch:
            beta = torch.arctan(np.tan(control.steering) * car.param.lr /
                                (car.param.lf + car.param.lr))
            ...
            beta = np.arctan(np.tan(control.steering) * car.param.lr /
                             (car.param.lf + car.param.lr))
    ...
    def advance_dynamics(state: CurvilinearState,
        ...
        if use_torch:
            beta = torch.arctan(torch.tan(control.steering) *
                                car.param.lr / (car.param.lf + car.param.lr))
