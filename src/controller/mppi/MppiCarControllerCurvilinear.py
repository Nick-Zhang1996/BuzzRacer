...
class MppiCarControllerCurvilinear(CarController):
    ...
    def prepareDiscretizedRaceline(self):
        ...
        drr = np.array(splev(ss % self.track.raceline_len_m, self.track.raceline_s, der=1))
        ...
        ddrr = vec_curvature = np.array(splev(ss%self.track.raceline_len_m,self.track.raceline_s,der=2))
        ...
        self.k_signed = np.copysign(curv, cross_curvature)
        self.k_signed = savgol_filter(self.k_signed, 15, 2)

        # describe track boundary as offset from raceline
        self.createBoundary()
        # ADDED self.ss TO THIS!!
        self.discretized_raceline = np.vstack(
            [self.raceline_points, self.raceline_headings, vv, self.raceline_left_boundary,
             self.raceline_right_boundary, self.ss, self.k_signed]).T
        return
    ...
    def initCuda(self):
        ...
        cuda_filename = "./controller/mppi/KinetoDynamic.cu"
    ...
    def control(self):
        ...
        curvilinearState = self.convertToCurvilinear(self.car.states, self.last_control)
        ...
        device_initial_state = self.to_device(curvilinearState)
    ...
    def convertToCurvilinear(self, state, control):
        x, y, heading, vx, vy, omega = state
        wheelbase = 0  # this could be ok, or it could be bad, idk
        _, n, refHeading, curvature, _, u = self.track.localTrajectory(state, wheelbase, True)
        xi = heading - refHeading
        zeta = float(self.track.uToS(u))
        # the below assumes that the car has reached the commanded throttle and steering
        curvilinearState = (control[0], control[1], vx, omega, zeta, n, xi, curvature)
