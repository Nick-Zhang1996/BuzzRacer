...
void forward_dynamics( float* state, float* u){
    ...
    if (vx < 0.05) {
        ...
        float Flr = MOTOR_PARAM_A * (throttle - vx / MOTOR_PARAM_B - MOTOR_PARAM_C) / 2.0;
        float frontWeight = (Flr * PARAM_H +PARAM_MASS * 9.8 * PARAM_LR) / PARAM_L;
        float rearWeight = (-Flr * PARAM_H +PARAM_MASS * 9.8 * PARAM_LF) / PARAM_L;

        float frontSlip = -atanf((vy + PARAM_LF * omega) / vx) + steering;
        float rearSlip = atanf((-vy + PARAM_LR * omega) / vx);

        float frontSlipForce = 0.5 * PARAM_D * sinf(PARAM_C * atanf(PARAM_B * frontSlip)) * frontWeight;
        float rearSlipForce = 0.5 * PARAM_D * sinf(PARAM_C * atanf(PARAM_B * rearSlip)) * rearWeight;

        d_vx = 2.0 * Flr; //vy * omega + - 2 / PARAM_MASS * frontSlipForce * sinf(steering)
        float d_vy = 2.0 / PARAM_MASS * (frontSlipForce * cosf(steering) + rearSlipForce) - vx * omega;
        d_omega = 2.0 * PARAM_LF / PARAM_IZ * frontSlipForce * cosf(steering) - 2.0 * PARAM_LR / PARAM_IZ * rearSlipForce;

//        slip_f = -atanf((omega * PARAM_LF + vy) / vx) + steering;
//        slip_r = atanf((omega * PARAM_LR - vy) / vx);
//
//        Ffy = 0.9 * tire_curve(slip_f) * PARAM_MASS * 9.8 * PARAM_LR / PARAM_L;
//        Fry = 1.15 * tire_curve(slip_r) * PARAM_MASS * 9.8 * PARAM_LF / PARAM_L;
//
//        d_vx = 6.17 * (throttle - vx / 15.2 - 0.333);
