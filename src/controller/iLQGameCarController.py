...
class iLQGameCarController(CarController):
    ...
    def update_dynamics(self,states,controls,dt=None):
        if (dt is None):
            dt = self.dt
        return self.dynamics.advanceKinematicBicycleDynamics(states.flatten(),controls.flatten(),dt,self.main.track)
    ...
    def lqControl(self,x0_i,x0_j):
        ...
        for iteration in range(self.iterations):
            ...
            for t in range(self.horizon):
                ...
                self.t.s('linearize')
                if (self.linearize_around_zero_control):
                    A, B = self.dynamics.linearizeManual(xx_i[-1],np.zeros(self.m),self.main.track)
                else:
                    A, B = self.dynamics.linearizeManual(xx_i[-1],u,self.main.track)
                self.t.e('linearize')
                xx_i.append(new_x.reshape(self.n,1))
                Ais.append(A)
                Bis.append(B)
                uu_i.append(u)
                ...
                self.t.s('linearize')
                if (self.linearize_around_zero_control):
                    A, B = self.dynamics.linearizeManual(xx_j[-1],np.zeros(self.m),self.main.track)
                else:
                    A, B = self.dynamics.linearizeManual(xx_j[-1],u,self.main.track)
                self.t.e('linearize')
                xx_j.append(new_x.reshape(self.n,1))
                Ajs.append(A)
                Bjs.append(B)
                uu_j.append(u)
    ...
    def drawTrajectory(self, traj=None, lineColor=(0,100,100)):
        ...
        if (self.main.visualization.update_visualization.is_set()):
            ...
            for t in range(self.horizon):
                curvi_states = traj[t]
                cart_states = self.dynamics.curv2CartTrack(curvi_states,self.main.track)
    ...
    def getCostMatrices(self,xx_i,uu_i,xx_j,uu_j,alpha=0):
        ...
        if (lead > 0 and v_diff < 0):
            ...
            Q1 = (1-alpha)*self.Q1 + alpha* np.diag(  [ 0,0.00,2.0,1.0,0]) # n:1
            q1 = (1-alpha)*self.q1 + alpha* np.array([[-4,0,0,0,0]]).T # -4
            ...
            if (np.abs(delta_x[0])<self.opponent_min_distance_s and np.abs(delta_x[2])<self.opponent_min_distance_n):
                ...
                if (False and np.abs(xx_i[t][0]-xx_j[t][0]) > self.opponent_min_distance_s):
                    if (xx_i[t][0] - xx_j[t][0] > 0):
                        ...
                        q1_x += (np.array([[sgn_s*2*self.opponent_min_distance_s, 0, sgn_n*2*self.opponent_min_distance_n, 0,0]]) @ Qcol1 @ II)
                else:
