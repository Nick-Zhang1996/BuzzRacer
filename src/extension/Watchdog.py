    ...
    def post_update(self):
        for car in self.main.cars:
            ...
            if (self.track.isOutside((x, y))):
                ...
                print_warning(
                    self.prefix()+'car outside track, terminating experiment')
            ...
            if (self.main.simulator.sim_t - car.laptimer.last_lap_ts > 20):
                ...
                )+'No new laps detected for %.2f s, terminating experiment' % (car.laptimer.last_laptime))
            if (car.laptimer.new_lap.is_set() and car.lap_count > 0):
                if (car.laptimer.last_laptime < 2.0):
                    self.triggered = True
                    self.main.exit_request.set()
                    print_warning(self.prefix(
                    )+'unreasonable laptime: %.2f, terminating experiment' % (car.laptimer.last_laptime))
