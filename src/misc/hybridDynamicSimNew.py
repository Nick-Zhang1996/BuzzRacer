...
class hybridDynamicSimNew(nn.Module):
    ...
    def forward(self, full_states, actions, enable_rnn):
        ...
        for i in range(self.forward_steps):
            ...
            Vy = -latest_state[:, 1] * \
                torch.sin(psi) + latest_state[:, 3]*torch.cos(psi)
            ...
            Vx = Vx + (forward_acc_r - lateral_acc_f *
                       torch.sin(steering) + Vy*omega) * self.dt
            Vy = Vy + (lateral_acc_r + lateral_acc_f *
                       torch.cos(steering) - Vx*omega) * self.dt
            # leading coeff = m/Iz
            omega = omega + self.m_div_Iz * \
                (lateral_acc_f * self.lf * torch.cos(steering) -
                 lateral_acc_r * self.lr) * self.dt
            ...
            debug_full_states = torch.cat(
                [debug_full_states, new_full_state], dim=1)
