...
class CarDataset(Dataset):
    ...
    def __init__(self, log_names, dt, history_steps, forward_steps):
        ...
        for filename in log_names:
            with open(filename, 'rb') as f:
                ...
                data_segment = np.array(
                    [x[:-1], vx, y[:-1], vy, heading[:-1], omega, throttle[:-1], steering[:-1]]).T
        ...
        self.full_states_std = np.sqrt(
            full_states_sq_sum / full_states_cnt - self.full_states_mean ** 2)
        ...
        for segment in self.raw_data:
            ...
            for i in range(len(segment)):
                ...
                # v>0.06m/s
                good_frame = (
                    abs(segment[i, 1])**2+abs(segment[i, 3])**2) > 0.1**2 and dot > 0.0
