...
import cv2
import cvxopt
from scipy.interpolate import splprep, interp1d
...
class QpSmooth(RCPTrack):
    ...
    def convert_to_spline(self):
        ...
        # s = smoothing factor
        # per = loop/period
        tck, u = splprep(r, u=np.linspace(
            0, self.track_length_grid, steps), s=0, per=1)
    ...
    def optimize_path(self,
        ...
        for iter_count in range(max_iter):
            ...
            if (visualize):
                img_track_rgb = cv2.cvtColor(
                    img_track.copy(), cv2.COLOR_BGR2RGB)
                plt.imshow(img_track_rgb)
                plt.show()
            if (save_gif):
        ...
        retval = self.generate_speed_profile(
            mu=0.8, acc_max_fun=lambda x: 5.0, dec_max_fun=lambda x: 3.3)
